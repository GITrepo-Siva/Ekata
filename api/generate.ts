...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (useGoogleSearch) {
            // For search-grounded responses, the output is text.
            // Wrap it in a JSON object for a consistent API response structure.
            res.status(200).json({ text: rawText });
            return;
        }

        try {
            // For schema-based requests, the AI should return a valid JSON string.
