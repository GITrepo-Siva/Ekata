...
const WeatherIcon: React.FC<{ code: number }> = ({ code }) => {
    ...
    return <span className="text-5xl" aria-hidden="true">{icon}</span>;
};

const WeatherDisplay: React.FC<{ weather: WeatherData }> = ({ weather }) => (
    <div className="flex flex-col sm:flex-row items-center justify-between text-center sm:text-left w-full">
...
const ClimateDashboard: React.FC = () => {
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLTableRowElement>, estate: CoffeeEstate) => {
    ...
    return (
        <Card>
            <h2 id="climate-heading" className="text-xl font-bold mb-4 text-brand-dark">Real-Time Estate Climate</h2>
            <div className="mb-4">
                <label id="estate-select-label" className="block text-sm font-medium text-gray-700 mb-1">
                    Select Coffee Estate
                </label>
                <div className="h-48 overflow-y-auto border rounded-lg bg-white">
                    <table className="w-full text-sm text-left" aria-labelledby="estate-select-label">
                        <tbody>
                            {COFFEE_ESTATES.map(estate => (
                                <tr 
                                    key={estate.name}
                                    onClick={() => setSelectedEstate(estate)}
                                    onKeyDown={(e) => handleKeyDown(e, estate)}
                                    tabIndex={0}
                                    role="button"
                                    aria-pressed={selectedEstate.name === estate.name}
                                    className={`cursor-pointer transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-brand-brown ${
