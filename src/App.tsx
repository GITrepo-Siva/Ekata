...
const App: React.FC = () => {
  ...
  const loadData = useCallback(async (customData?: PriceData[]) => {
    setPriceLoadingState(LoadingState.LOADING);
    
    // Only load competitor data on a full refresh (not custom data upload)
    if (!customData) {
    ...
    try {
      ...
      if (customData) {
        historicalData = customData;
      } else {
        const [apiHistoricalData, competitorPrices] = await Promise.all([
          fetchCoffeePrices(),
  ...
  const handleCustomDataUpload = (data: PriceData[]) => {
    loadData(data);
  };
  ...
  return (
    <div className="min-h-screen bg-brand-light-gray text-brand-dark font-sans flex flex-col">
      <Header 
        onRefresh={() => loadData()} 
        onDataUploaded={handleCustomDataUpload}
